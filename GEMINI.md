...
- **フェーズ 1: 要件定義・対話による仕様決定** ✅ (完了)
  - **合意済みの要件**:
    - **クイズルール**: 4択クイズ、2択タイムアタック（10問）、2択エンドレス（1問3秒）の3つのモードを搭載。未解放（正解3回未満）の犬種を優先的に出題するロジック。
    - **出題モード選択**: 「おなじみの犬種」と「すべての犬種（全犬種モード）」の2種類から出題範囲を選択可能（スタート画面にそれぞれの総数を動的表示）。
    - **難易度選択**: 「かんたん（ランダム出題）」と「むずかしい（見た目が似た犬種を不正解の選択肢に優先表示）」の2段階。
    - **マルチヒント機能**: 4択クイズの「むずかしい」モード時、ヒント（おやつ）ボタン押下で「ハズレの選択肢が2つ消える（3回目使用時）」「頭文字テキストヒント表示（4回目使用時）」が段階的に発動。
    - **図鑑システム**: クイズ正解数で3段階解放（1回: 名前とカラー写真、図鑑から直接タイムアタックを開始する機能の解放、2回: 生物情報[原産国・大きさ]、3回: 日本語豆知識の解説文）。
    - **効果音システム**: クイズ回答時にブラウザ合成音（Web Audio API）による「ピンポーン（正解）」「ブッブー（不正解）」の効果音を再生。
    - **デザイン**: かわいらしく親しみやすいデザイン（ベージュ・優しいイエロー基調、肉球ローディング、カードのうねり演出など。2択クイズ時は画像を限界まで大きく表示）。
- **フェーズ 2: 基本設計・画面設計** ✅ (完了)
- **フェーズ 3: 開発・実装** ✅ (完了)
- **フェーズ 4: 動作確認・テスト** ✅ (完了)
...
- アプリケーションの全体改善リリースの実装
  - スタート画面に効果音のオン/オフおよび音量スライダー設定UIを追加（`index.html`, `style.css`, `app.js`）。
  - 効果音再生時に `soundMuted` および `soundVolume` を適用するよう合成エンジンを拡張（`game.js`, `state.js`）。
  - クイズ中断の `confirm()` ダイアログを廃支し、可愛いカスタムポップアップモーダルを自作・制御（`index.html`, `style.css`, `ui.js`, `game.js`, `app.js`）。
  - 4択クイズの「おやつ（ヒント）」機能を4段階化し、1〜3回目でもハズレ選択肢が1つずつ消えるように改良（`game.js`）。
  - 2択クイズの画像を画面幅580px以上の環境で自動的に「左右（横並び）」にするレスポンシブ表示に対応（`style.css`）。
  - 図鑑画面の下部にセーブデータのインポート・エクスポートができる「データ管理」エリアを新設（`index.html`, `style.css`, `ui.js`, `app.js`）。
  - タイムアタック結果画面で「30問クリア」と古いテキストが表示されていたバグを「10問クリア」に修正（`game.js`）。
  - DogAPIとのキー名の不整合をマッピング辞書 `DOG_API_KEY_MAP` として整理し、ハードコーディングを排除（`dictionary.js`, `api.js`）。
  - アプリの説明、ファイル構成、起動方法などをやさしく解説した `README.md` を新規作成。

### v1.8.0 (2026-06-08)
- 2択クイズのレイアウト縦並び化と不正解犬種名の表示機能の実装
  - 2択クイズ（タイムアタック・エンドレス）の画像表示を、左右並びから上下並びに変更（`style.css`）。
  - 縦並び時に画像が画面からはみ出さないよう、高さ制限（160px）と余白の再調整を実施（`style.css`）。
  - 画像コンテナ内に不正解犬種名表示用の `div` 要素を追加し、`ui.js` に参照を登録（`index.html`, `ui.js`）。
  - `game.js` の `selectAnswer2Choices` を修正し、回答後に正解ではない（不正解）側の画像の上に本来の犬種名を半透明で表示するロジックを実装。また、次のお題表示時に表示をクリアする処理を追加。

### v1.7.1 (2026-06-08)
- 2択画像の表示サイズ調整とバグ修正
  - クイズカード要素（`elQuizCard`）の取得方法をゲッターに変更し、DOM構築タイミングによる取得漏れバグを修正（`ui.js`）。
  - カードの余白、ボタンの枠線、ラベルのスペースを極限まで削減し、2択写真がカード幅いっぱいに最大表示されるようにCSSを微調整（`style.css`）。

### v1.7.0 (2026-06-08)
- 既知の問題点の修正と全体的な機能改善
  - **タイムアタックのフリーズ対策**: 画像ロード時の5秒タイムアウト処理（`api.js`）と、画像の1枚ずつ順次ダウンロード化（`game.js`）により通信のハングアップを解消。
  - **難易度表記の修正**: スタート画面の難易度選択から「ぼかし」の表現を削除し、実際の仕様に合わせて「かんたん」「むずかしい（似た犬種）」に変更（`index.html`）。
  - **2択画像の大型化**: 2択クイズ時にカードとボタンの余白を縮小するスタイルを追加し、写真を約1.3倍に拡大（`style.css`, `ui.js`, `game.js`）。
  - **戻るボタンの修正**: 図鑑画面の「スタートがめんに戻る」ボタンのテキストを「スタート画面に戻る」に修正（`index.html`）。
  - **犬種の総数表示**: 出題モード選択肢に「おなじみの犬種 (25種)」「全犬種モード (117種)」のように総数を動的表示（`index.html`, `app.js`）。
  - **効果音の実装**: Web Audio APIによる電子音の合成により、正解時「ピンポーン」、不正解時「ブッブー」の効果音を遅延なしで再生（`game.js`）。
...
- 画像の事前プリロード（先読み）機能と開始前「じゅんび画面」の導入
  - ゲーム開始前に、各モードに必要な全画像（4択:10枚、タイムアタック:20枚、エンドレス:初期20問分40枚）を並列でロード・キャッシュする処理を実装。
  - 最低3秒間ターゲット犬種の見本（タイムアタック時）やローディングを表示する「じゅんび画面」を追加。
  - 準備完了後のカウントダウン（3, 2, 1, スタート！）を経てゲームが始まることで、タイムアタックで最初の画像が表示される前にタイマーが開始される不具合を解消。
- エンドレスモードの画像ロード不具合解消
  - 開始前にあらかじめ20問分の画像をキャッシュし、ゲーム中は1問回答するごとにバックグラウンドで次の1問を非同期でロード・補充するキュー方式に刷新。
  - プレイ中に画像ロードによる画面の引っ掛かりや非表示になるバグを解消。
- クイズ難易度「むずかしい」の調整
  - 従来の「ぼかし（モザイク）」効果を廃止し、お題犬種と見た目が似ている犬種を優先的にハズレ選択肢に表示する方式に改修。
  - `dictionary.js` に「見た目が似ている犬種のグループリスト（`SIMILAR_DOG_GROUPS`）」を新規定義。
- 図鑑から直接2択タイムアタックを開始する機能の追加
  - 図鑑でカラー写真が解放されたカード（正解回数1回以上）に「2択で遊ぶ ⏱️」ボタンを追加。
  - ボタンをクリックすると、その犬種がターゲットに固定されたタイムアタックが即座に開始されるように実装。
- Playwright E2Eテストの適合とタイムアウト調整
  - 準備画面のウェイトを考慮し、テストケース2, 3, 5, 6, 7 に `#quiz-main-contents` の表示完了待機処理を追加。
  - テスト6のタイムアウト時間を60秒に延長し、全体のテストが正常に通過することを確認。
